...
Type Type::strippedTo(Type const& _t) const
{
	...
	while (ret.m_top)
	{
		...
		for (ModifyingType* i = ret.m_top->tryKind<ModifyingType>(); i && j; i = i->childType()->tryKind<ModifyingType>(), j = j->childType()->tryKind<ModifyingType>())
			if (i->kind() != j->kind())
				goto BAD;
		if (!j)
			break;
...
bool Type::isUltimatelyNull() const
{
	...
	while (t->isKind<ModifyingType>())
		t = t->asKind<ModifyingType>()->childType();
