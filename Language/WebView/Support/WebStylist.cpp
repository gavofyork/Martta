...
void WebStylist::setProperties(Hash<String, String> const& _p)
{
	foreach (String s, _p.keys())
		...
		else
		{
			...
			{
				m_css = _p[s].toBool() ?
					"body { color: black; font-size: 12px; font-family: Mono; background-color: rgba(255, 255, 255, 1); -khtml-user-select: none; }"
				...
				:
					"body { color: #666; font-size: 12px; font-family: Sans; background-color: rgba(255, 255, 255, 1); -khtml-user-select: none; }"
					".layout { display: inline-block; }"
					".keyword { font-weight: bold; }"
					"._ms_unreal { color: #888; }"
					"._ms_minor { color: #888; font-size: 10px; }"
					".unreal { color: #888; }"
					".minor { color: #888; font-size: 10px; }"
					".symbol { font-weight: bold; }"
					".block { display: block; position: relative; margin-left: 20px; }"
					".deblock { display: block; position: relative; margin-left: -20px; }"
				//	".editing { background-color: rgba(255, 0, 0, 0.25); }"
