...
{
	if (!childIs<TypeNamer>(_i))
		return Type();
	Type p(childAs<TypeNamer>(_i)->apparentType());
...
Type TypedOwner::canonicalType(int _i) const
{
	if (!childIs<TypeNamer>(_i))
		return Type();
	return childAs<TypeNamer>(_i)->type();
}
...
//	child(_i)->debugTree();
	...
	foreach (Type i, deniedTypes(_i))
		if (childAs<TypeNamer>(_i)->apparentType() == i)
			return false;
	foreach (Type i, allowedTypes(_i))
		if (!childIs<TypeNamer>(_i))
		{
			...
			if (childAs<TypeNamer>(_i)->apparentType().isSimilarTo(i, TypeConcept::Convertible))
				return true;
			else
			{
				child(_i)->debugTree();
				mDebug() << childAs<TypeNamer>(_i)->apparentType()->code() << " != " << i->code();
			}
	return false;
}
...
{
	int num = max(minRequired(), cardinalChildCount());
	List<int> ai;
	...
	if (ai.count() || allowedTypes(num).count())
	{
		...
		foreach (int i, ai)
		{
			Pairs q(indexName(i) + L": " + escape(effectiveType(i)->typeName()));
